...
from .random_utils import randint, random, uniform, randbool
...
def generate_data(values: dict, start_date: int, end_date: int) -> None:
    ...
    values['timestamp'] = [ts+randint(-10, 10) for ts in range(start_date, end_date, 100)]
    if 'battery' in values:
        generate_battery(numbers=values['battery'], timestamps=values['timestamp'])
    if 'temperature' in values:
        generate_temperature(numbers=values['temperature'], timestamps=values['timestamp'])
    if 'humidity' in values:
        generate_humidity(numbers=values['humidity'], timestamps=values['timestamp'])
    if 'cold' in values:
        generate_cold(numbers=values['cold'], timestamps=values['timestamp'])
    if 'freezer' in values:
        generate_freezer(numbers=values['freezer'], timestamps=values['timestamp'])
    if 'ultrafreezer' in values:
        generate_ultrafreezer(numbers=values['ultrafreezer'], timestamps=values['timestamp'])
