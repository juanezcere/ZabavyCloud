...
class PerlinNoise(object):
    ...
    def __init__(self, seed: Optional[int] = None, octaves: float = 1, frequency: float = 1.0, amplitude: float = 1.0):
        """
    ...
    def noise(self, coordinates: Union[int, float, Iterable]) -> float:
        ...
        coordinates = list(
            map(lambda coordinate: (coordinate / self.frequency) * self.octaves, coordinates),
        )
