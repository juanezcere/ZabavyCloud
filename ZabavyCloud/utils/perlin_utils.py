...
class PerlinNoise(object):
    ...
    def __init__(self, seed: Optional[int] = None, octaves: float = 1, frequency: float = 100.0, amplitude: float = 1.0):
        """
        ...
        Parameters:
            ...
            * frequency : optional positive float, default = 1.0
                positive number to devide the angle
            * amplitude : optional positive float, default = 1.0
                positive number of amplitude of the function
        """
        if octaves <= 0:
    ...
    def __call__(self, coordinates: Union[int, float, Iterable]) -> float:
        ...
        Returns:
            noise_value
        """
        return self.noise(coordinates)*self.amplitude
    ...
    def noise(self, coordinates: Union[int, float, Iterable]) -> float:
        ...
        coordinates = list(
            map(lambda coordinate: (coordinate / self.frequency)
                * self.octaves, coordinates),
        )
